...
class CorrelationIdTests(unittest.TestCase):
    def test_dispatch_client_forwards_correlation_id(self) -> None:
        captured = {}
        ...
        def fake_urlopen(req):
            captured["correlation"] = req.get_header("X-correlation-id")
        ...
        with patch("paperclip.dispatch.request.urlopen", fake_urlopen):
            client.submit({"task": {"id": "task_123"}}, correlation_id="corr-123")

        self.assertEqual(captured["correlation"], "corr-123")

    def test_paperclip_returns_correlation_id_header(self) -> None:
        ...
        conn.request("POST", "/tasks", body=payload, headers={"Content-Type": "application/json", "Content-Length": str(len(payload)), "X-Correlation-ID": "corr-paperclip"})
        response = conn.getresponse()
        response.read()
        self.assertEqual(response.getheader("X-Correlation-ID"), "corr-paperclip")
        conn.close()
        server.server_close()
        thread.join(timeout=2)

    def test_hermes_returns_generated_correlation_id_header(self) -> None:
