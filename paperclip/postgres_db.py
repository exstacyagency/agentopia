...
from scripts.postgres_schema import apply_postgres_migrations
...
class PostgresPaperclipDB:
    ...
    def _ensure_schema(self) -> None:
        with self.conn.transaction():
            apply_postgres_migrations(self.conn)
    ...
    def update_task_state(self, task_id: str, state: str, updated_at: str, approval_status: str | None = None) -> None:
        if approval_status is None:
            ...
            self.conn.execute(
                "UPDATE tasks SET state = %s, approval_status = %s, updated_at = %s WHERE id = %s",
    ...
    def add_audit_event(self, task_id: str, event_type: str, actor: str, payload: dict, created_at: str) -> None:
        self.conn.execute(
            "INSERT INTO audit_events (task_id, event_type, actor, payload_json, created_at) VALUES (%s, %s, %s, %s, %s)",
    ...
    def get_audit_events(self, task_id: str) -> list[dict]:
        rows = self.conn.execute(
            "SELECT id, task_id, event_type, actor, payload_json, created_at FROM audit_events WHERE task_id = %s ORDER BY id ASC",
            (task_id,),
        ).fetchall()
        events: list[dict] = []
        for row in rows:
    ...
    def enqueue_task(self, task_id: str, correlation_id: str | None, created_at: str, max_attempts: int = 3) -> None:
        self.conn.execute(
            "INSERT INTO task_queue (task_id, status, correlation_id, attempt_count, max_attempts, next_attempt_at, started_at, timeout_at, worker_id, lease_expires_at, last_error, created_at, updated_at) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s) ON CONFLICT (task_id) DO UPDATE SET status = EXCLUDED.status, correlation_id = EXCLUDED.correlation_id, attempt_count = EXCLUDED.attempt_count, max_attempts = EXCLUDED.max_attempts, next_attempt_at = EXCLUDED.next_attempt_at, started_at = EXCLUDED.started_at, timeout_at = EXCLUDED.timeout_at, worker_id = EXCLUDED.worker_id, lease_expires_at = EXCLUDED.lease_expires_at, last_error = EXCLUDED.last_error, created_at = EXCLUDED.created_at, updated_at = EXCLUDED.updated_at",
    ...
    def claim_queue_item(self, task_id: str, worker_id: str, lease_expires_at: str, updated_at: str) -> None:
        self.conn.execute(
            "UPDATE task_queue SET worker_id = %s, lease_expires_at = %s, updated_at = %s WHERE task_id = %s",
    ...
    def mark_queue_running(self, task_id: str, started_at: str, timeout_at: str, updated_at: str) -> None:
        self.conn.execute(
            "UPDATE task_queue SET status = %s, started_at = %s, timeout_at = %s, updated_at = %s WHERE task_id = %s",
    ...
    def mark_queue_timed_out(self, task_id: str, last_error: str, updated_at: str) -> None:
        self.conn.execute(
            "UPDATE task_queue SET status = %s, last_error = %s, updated_at = %s WHERE task_id = %s",
            ("timed_out", last_error, updated_at, task_id),
        )

    def reset_queue_to_queued(self, task_id: str, updated_at: str) -> None:
        self.conn.execute(
            "UPDATE task_queue SET status = %s, worker_id = NULL, lease_expires_at = NULL, started_at = NULL, timeout_at = NULL, updated_at = %s WHERE task_id = %s",
    ...
    def mark_queue_retry(self, task_id: str, attempt_count: int, next_attempt_at: str, last_error: str, updated_at: str) -> None:
        self.conn.execute(
            "UPDATE task_queue SET status = %s, attempt_count = %s, next_attempt_at = %s, last_error = %s, updated_at = %s WHERE task_id = %s",
    ...
    def mark_queue_dead_letter(self, task_id: str, attempt_count: int, last_error: str, updated_at: str) -> None:
        self.conn.execute(
            "UPDATE task_queue SET status = %s, attempt_count = %s, last_error = %s, updated_at = %s WHERE task_id = %s",
            ("dead_letter", attempt_count, last_error, updated_at, task_id),
        )

    def mark_queue_dispatched(self, task_id: str, updated_at: str) -> None:
        self.conn.execute(
            "UPDATE task_queue SET status = %s, updated_at = %s WHERE task_id = %s",
    ...
    def get_queue_item(self, task_id: str) -> dict | None:
        row = self.conn.execute(
            "SELECT task_id, status, correlation_id, attempt_count, max_attempts, next_attempt_at, started_at, timeout_at, worker_id, lease_expires_at, last_error, created_at, updated_at FROM task_queue WHERE task_id = %s",
            (task_id,),
        ).fetchone()
    ...
    def list_queue_items(self, status: str | None = None) -> list[dict]:
        if status is None:
            ...
            rows = self.conn.execute(
                "SELECT task_id, status, correlation_id, attempt_count, max_attempts, next_attempt_at, started_at, timeout_at, worker_id, lease_expires_at, last_error, created_at, updated_at FROM task_queue WHERE status = %s ORDER BY created_at ASC",
                (status,),
            ).fetchall()
    ...
    def create_idempotency_record(self, idempotency_key: str, task_id: str, created_at: str) -> None:
        self.conn.execute(
            "INSERT INTO task_idempotency (idempotency_key, task_id, created_at) VALUES (%s, %s, %s)",
            (idempotency_key, task_id, created_at),
        )

    def get_idempotent_task_id(self, idempotency_key: str) -> str | None:
        row = self.conn.execute(
            "SELECT task_id FROM task_idempotency WHERE idempotency_key = %s",
            (idempotency_key,),
        ).fetchone()
    ...
    def store_result(self, task_id: str, payload: dict, created_at: str) -> None:
        self.conn.execute(
            "INSERT INTO task_results (task_id, result_payload, created_at) VALUES (%s, %s, %s) ON CONFLICT (task_id) DO UPDATE SET result_payload = EXCLUDED.result_payload, created_at = EXCLUDED.created_at",
            (task_id, json.dumps(payload), created_at),
        )

    def get_result(self, task_id: str) -> dict | None:
        row = self.conn.execute("SELECT result_payload, created_at FROM task_results WHERE task_id = %s", (task_id,)).fetchone()
        if row is None:
            return None
        return {
    ...
    def delete_task_data(self, task_id: str) -> None:
        def operations(db: PostgresPaperclipDB) -> None:
            db.conn.execute("DELETE FROM task_idempotency WHERE task_id = %s", (task_id,))
            db.conn.execute("DELETE FROM task_results WHERE task_id = %s", (task_id,))
            db.conn.execute("DELETE FROM task_queue WHERE task_id = %s", (task_id,))
            db.conn.execute("DELETE FROM audit_events WHERE task_id = %s", (task_id,))
            db.conn.execute("DELETE FROM tasks WHERE id = %s", (task_id,))
